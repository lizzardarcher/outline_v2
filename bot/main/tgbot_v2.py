...
bot = TeleBot(TelegramBot.objects.get(pk=1).token)
bot.parse_mode = 'HTML'
...
def start(message):
    if message.chat.type == 'private':
        ...
        try:
            ...
            bot.send_message(chat_id=message.chat.id, text=msg.new_user_bonus)
        except:
            ...
        bot.send_message(chat_id=message.chat.id, text=msg.start_message.format(message.from_user.first_name),
                         reply_markup=markup.get_app_or_start())
        # bot.send_message(chat_id=message.chat.id, text=msg.main_menu_choice, reply_markup=markup.start())
        # bot.send_message(chat_id=message.chat.id, text=msg.main_menu_choice, reply_markup=markup.get_app_or_start())
        ...
        if 'start=' in message.text:
            ...
            if not same_user_check:
                try:
                    ...
                    bot.send_message(chat_id=message.chat.id, text=msg.main_menu)
                    bot.send_message(chat_id=message.chat.id, text=msg.main_menu_choice,
                                     reply_markup=markup.start())
                except:
                    logger.error(f'{traceback.format_exc()}')

            else:
                bot.send_message(chat_id=message.chat.id, text=msg.referral_bond_error)
                bot.send_message(chat_id=message.chat.id,
                                 text=msg.start_message.format(message.from_user.first_name))
                bot.send_message(chat_id=message.chat.id, text=msg.main_menu)
                bot.send_message(chat_id=message.chat.id, text=msg.main_menu_choice, reply_markup=markup.start())
            ...
            if user.top_up_balance_listener:
                ...
                try:
                    ...
                    if amount >= 150:
                        bot.send_message(chat_id=message.chat.id, text=msg.start_payment.format(str(amount)),
                    ...
                    bot.send_message(chat_id=message.chat.id, text=msg.start_payment_error.format(message.text),
                                     reply_markup=markup.back())
                    logger.error(f'{traceback.format_exc()}')
...
def checkout(pre_checkout_query):
    bot.answer_pre_checkout_query(pre_checkout_query.id, ok=True, error_message=msg.payment_unsuccessful)
...
def got_payment(message):
    ...
    bot.send_message(chat_id=message.chat.id, text=msg.payment_successful.format(amount, currency))
    bot.send_message(chat_id=message.chat.id, text=msg.main_menu_choice, reply_markup=markup.start())
    ...
    if referred_list:
        for r in referred_list:
            ...
            if percent:
                ...
                bot.send_message(user_to_pay.user_id,
...
def callback_query_handlers(call):
    ...
    def send_dummy():
        bot.send_message(call.message.chat.id, text=msg.dummy_message, reply_markup=markup.start())
    ...
    if call.message.chat.type == 'private':
        ...
        if 'download_app' in data:
            bot.send_message(call.message.chat.id, text=msg.download_app, reply_markup=markup.download_app())

        elif 'app_installed' in data:
            bot.send_message(chat_id=call.message.chat.id, text=msg.app_installed, reply_markup=markup.start())

        elif 'manage' in data:
            bot.send_message(call.message.chat.id, msg.avail_location_choice,
            ...
            if user.subscription_status:
                ...
                if country:
                    try:
                        ...
                        bot.send_message(call.message.chat.id, text=f'{msg.key_avail}\n<code>{key}</code>',
                                         reply_markup=markup.key_menu(country))
                    except:
                        logger.error(f'{traceback.format_exc()}')
                        bot.send_message(call.message.chat.id, text=msg.get_new_key,
                                         reply_markup=markup.get_new_key(country))
            else:
                bot.send_message(call.message.chat.id, msg.no_subscription,
            ...
            if 'get_new_key' in call.data:
                ...
                bot.send_message(call.message.chat.id, text=msg.paymemt_menu, reply_markup=markup.paymemt_menu())

            elif 'buy_subscripton' in data:
                bot.send_message(call.message.chat.id, text=msg.choose_subscription,
                                 reply_markup=markup.choose_subscription())

            elif 'payment' in data:
                if 'ukassa' in data:
                    bot.send_message(call.message.chat.id, text=msg.top_up_balance)
                    TelegramUser.objects.filter(user_id=user.user_id).update(top_up_balance_listener=True)
                elif 'usdt' in data:
                    bot.send_message(call.message.chat.id, text=msg.usdt_message,
                ...
                if user_balance < price:
                    bot.send_message(call.message.chat.id, text=msg.low_balance,
                                     reply_markup=markup.top_up_balance())
                else:
                    description = f' <code>{days}</code> за <code>{price}р.</code>'
                    bot.send_message(call.message.chat.id, text=msg.confirm_subscription.format(description),
                ...
                bot.send_message(call.message.chat.id, text=msg.sub_successful.format(new_exp_date, data[-2]),
                                 reply_markup=markup.proceed_to_profile())
        ...
        # todo  Разобраться с остатками ГБ
            ...
            # try:
            #     update_keys_data_limit(user=user)
            # except:
            #     print(traceback.format_exc())
            ...
            try:
                ...
                if user.income >= 500:
                    ...
                    bot.send_message(call.message.chat.id, text=msg.withdraw_request.format(str(user.income)),
                                     reply_markup=markup.proceed_to_profile())
                    logger.info(
            ...
            bot.send_message(call.message.chat.id, text=msg.help_message, reply_markup=markup.start(),
                             parse_mode='HTML')

        elif 'popup_help' in data:
            bot.answer_callback_query(call.id, text=msg.popup_help, show_alert=True)

        elif 'common_info' in data:
            bot.send_message(call.message.chat.id, text=msg.commom_info, reply_markup=markup.help_markup())

        elif 'back' in data:
            bot.send_message(chat_id=call.message.chat.id, text=msg.main_menu_choice, reply_markup=markup.start())


bot.infinity_polling(timeout=100, skip_pending=True)  # TELEGRAM BOT
